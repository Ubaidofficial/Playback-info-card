...
* **Admin session controls (Stop / Send Message)**: visible only to server administrators, and both actions call Jellyfin's own native Session API (`Sessions/{id}/Playing/Stop`, `Sessions/{id}/Message`) via the shared `ApiClient` — the same endpoints Jellyfin-web's own remote-control UI uses. This plugin does not implement its own authorization for these actions; Jellyfin core's own endpoint-level permission check is the real enforcement boundary, and client-side hiding of the buttons is a convenience layer on top of it, not a substitute for it.
