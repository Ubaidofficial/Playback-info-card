...
The page resolves URLs through Jellyfin's own `ApiClient.getUrl()`, so it works behind a sub-path (`/jellyfin`, `/media`, etc.) with no special rewrite rules. Standard Jellyfin proxy config (WebSocket plus HTTP passthrough) is enough, just don't cache `/Sessions` or `/web/configurationpage` responses.

## Rollback

Every release ever published stays on [GitHub Releases](https://github.com/Ubaidofficial/Playback-info-card/releases). The catalog (`manifest.json`) only surfaces the 5 most recent for the in-app updater, but nothing is ever deleted.
...
- The monitor never tries to guess Wi-Fi, Ethernet, or cellular. Jellyfin doesn't reliably expose that, and network fingerprinting isn't something this plugin does.
...
- When "Include Poster Image" is on and the item's poster is cached locally, it's uploaded directly as part of the same request (Discord uses `files[0]` plus `payload_json`, Telegram uses `sendPhoto`), never a link back to your server, so it works even if your Jellyfin isn't internet reachable. Missing or oversized (over 8MB) images just fall back to a text-only message.
